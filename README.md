...
一个用于整理、清理、重新编号和重建考试题目 PowerPoint 的通用 Codex Skill，适用于不同年级、Form、年份、科目、课程体系和语言。
...
- 每个指定分组（例如章节、单元、主题或题型）从 `1.` 开始重新编号
- 按用户指定的分类轴整理题目，不预设年级或科目
...
```text
使用 $exam-ppt-sorter 把题目按指定的章节、单元、主题或题型整理成学生教材。先锁定主文件的年级或 Form、年份、科目、课程体系和语言；参考文件只提供排版或标签证据，不得成为内容来源。移除旧主序号，每个指定分组重新从 1 编号；标签放在题目右下角；一页尽量放多题，但不要裁题、拆题或过度缩小。完成全部对账和逐页 QA 后再交付。
```
...
3. 对代表性图片进行小批量清理测试。
4. 批量清理并检查印刷内容是否保持不变。
5. 对照参考资料修正用户指定的分类字段和来源标签，但不得从参考文件导入题目内容。
6. 安全移除旧主序号并按指定分组重新编号。
